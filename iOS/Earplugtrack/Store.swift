...
final class Store: ObservableObject {
    ...
    private let fileName = "earplugtrack_entries.json"
    ...
    private func seed() {
        entries = [
            Entry(date: Calendar.current.date(byAdding: .day, value: -1, to: Date())!, flag: false, rating: 4, note: ""),\nEntry(date: Calendar.current.date(byAdding: .day, value: -2, to: Date())!, flag: true, rating: 5, note: ""),\nEntry(date: Calendar.current.date(byAdding: .day, value: -3, to: Date())!, flag: false, rating: 3, note: "")
        ]
    }
    private func load() {
